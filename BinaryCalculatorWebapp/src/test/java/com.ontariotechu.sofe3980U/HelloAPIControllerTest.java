...
public void EmailAPINoParameters() throws Exception {
    this.mvc.perform(get("/emailAPI"))
        .andExpect(status().isOk())
        .andExpect(MockMvcResultMatchers.jsonPath("$.name").value("John Doe")) // Explicitly checking the default
        .andExpect(MockMvcResultMatchers.jsonPath("$.suggestedEmail").value("John.Doe@OntarioTechU.net"));
}

@Test
public void EmailAPIWithFirstNameOnly() throws Exception {
    this.mvc.perform(get("/emailAPI").param("fname","Jack"))
        .andExpect(status().isOk())
        .andExpect(MockMvcResultMatchers.jsonPath("$.name").value("Jack Doe")) // Still relying on default last name, but clear about it
        .andExpect(MockMvcResultMatchers.jsonPath("$.suggestedEmail").value("Jack.Doe@OntarioTechU.net"));
}

@Test
public void EmailAPIWithLastNameOnly() throws Exception {
    this.mvc.perform(get("/emailAPI").param("lname","Sparrow"))
        .andExpect(status().isOk())
        .andExpect(MockMvcResultMatchers.jsonPath("$.name").value("John Sparrow"))  // Still relying on default first name, but clear about it
